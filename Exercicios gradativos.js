...
const impNumbers = randomNumbers.filter( randomNumber => randomNumber % 2 == 1);
...
const numbersUnder501 = crazyNumbers.reduce((accumulator, crazyNumber) => {
	if(crazyNumber < 501) {
...
const expNumbers = numbers.map( number => number ** number);
...
const moviesUnder2000 = tarantinoMovies.filter( tarantinoMovie => tarantinoMovie.release < 2000);

console.log(moviesUnder2000);
/*
  05
...
const nameCart = cart.reduce((accumulator, cart) => {
 return  accumulator + '-' + cart.name + '\n'
}, '');
